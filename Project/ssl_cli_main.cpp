...
void *ssl_thread( void * thread_arg )
{
	...
	while ( 1 )
	{
		...
		if ( !( rli.line % 1000 ) )
		{
			//printf( "Process ID %d Received back %ld lines (total %ld bytes, sem_limit %d)\n",getpid(), rli.line, rli.total, sem_limit );
			sem_wait(countchecker);
			(*sharmem_total)+=1000;
			
			
			sem_post(countchecker);
		}
...
void ssl_fork( sockaddr_in *addr, int avg_line_len )
{
	...
	{
		Message smsg;
		smsg.type = SSL_CONNECTED;
		smsg.val = getpid();
		ErrorCheck((msgsnd( msgid, &smsg, MsgSize, 0 )));
		printf("Child %d Sent SSL confirmation\n",getpid());
		
		
		Message rmsg;
		printf("Waiting for GO confirmation from parent\n");
		ErrorCheck( msgrcv( msgid, &rmsg, MsgSize,GO, 0 ));
		printf("Received GO from parent\n");
		
		
		
		//printf( "Would be Press Enter to start test. Child %d ready to go\n" ,getpid());
		//getchar();
	}
...
int main ( int argn, char ** args )
{
	...
	msgid = msgget( 0x40b0, 0600 | IPC_CREAT );
	// help?
	...
	int semval;
	countchecker = sem_open( CHECKSEMNAME, O_CREAT , 0660,1 );
	sem_getvalue(countchecker,&semval);
    printf( "Initial countchecker: %d\n", semval );
	long sharmem_total_key = getuid();
	long sharmem_total_id = shmget(sharmem_total_key,sizeof(long),0644|IPC_CREAT);
	sharmem_total=(long*)shmat(sharmem_total_id,NULL,0);
	
	long sharmem_prev_key = getuid()+1;
	long sharmem_prev_id = shmget(sharmem_prev_key,sizeof(long),0644|IPC_CREAT);
	sharmem_prev=(long*)shmat(sharmem_prev_id,NULL,0);
	
	(*sharmem_total) = 0;
	(*sharmem_prev) = 0;
	
	printf("Shared set to %ld %ld\n",*sharmem_total,*sharmem_prev);
	...
	//	Message rmsg;
	//	printf("Waiting for SSL confirmation from child %d\n",pid);
	//	ErrorCheck( msgrcv( msgid, &rmsg, MsgSize,SSL_CONNECTED, 0 ));
	//	printf("Received SSL confirmation from child %d\n",pid);
	...
	for ( int i = 0; i < num_fork; i++ )
	{
		...
		smsg.val = getpid();
		ErrorCheck((msgsnd( msgid, &smsg, MsgSize, 0 )));
		printf("Sent GO confirmation to child %d\n",i);
	...
	while(1)
		
		{
			...
			{
				sem_wait(countchecker);
				printf("Parent raised checkpoint after %d seconds!!\n",elapsedTime);
				printf("Processed %ld lines in total.\n",*sharmem_total);
				printf("Processed %ld NEW lines in total.\n",*sharmem_total - *sharmem_prev);
				
				
				
				*sharmem_prev = *sharmem_total;
	...
	sem_unlink (CHECKSEMNAME);
	shmctl(sharmem_total_id, IPC_RMID, NULL);
	shmctl(sharmem_prev_id, IPC_RMID, NULL);
}
/* EOF - ssl_cli.cpp */
