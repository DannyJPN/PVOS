...
#define CHK_NULL(x) if ((x)==NULL) {printf("SSL_NULL\n");exit (1);}
...
void*handleclient(void*args)
{
       ...
       /*  pollfd l_readpoll[ 1 ];
		...
		err = SSL_accept (ssl);                        CHK_SSL(err); ErrorCheck(err);
		  ...
		  /* Get client's certificate (note: beware of dynamic allocation) - opt */
			...
			while ( ACTIVE )
			{
				...
				//printf("Thread %lu going to send buf %s\n",conn->tid,buf);
				err = SSL_write( ssl, buf, strlen( buf ) );		//printf("Thread %lu error %d\n",conn->tid,err);		
				CHK_SSL(err);
				if(err<0)
				{
				//	printf("Thread %lu SSL error %d\n",conn->tid ,SSL_get_error(ssl,err));break;
				}
				
				//printf("Thread %lu sent buf\n",conn->tid);
			}
		
			/* Clean up. */
					

printf("Thread %lu going to shut,closing socket %d\n",conn->tid,conn->sock);
 SSL_free (ssl); 
ErrorCheck(close(conn->sock));


printf("Thread %lu shutdown\n",conn->tid);
pthread_exit(0);
}
