...
*/
            ...
            inread.fd = conn->sock;
			log_msg( LOG_INFO, "Process %d started operating at socket %d",getpid(),conn->sock );
			...
			while(ACTIVE)
			{
				...
				if ( FD_ISSET( conn->sock, &sockset ) )
				{
					...
					if ( !l )
					{
							...
							log_msg( LOG_INFO, "Connection closed. Process %d terminating. Socket %d closing" ,getpid(),conn->sock);
			...
			free(conn);
            //pids.erase(remove(pids.begin(), pids.end(), getpid()), pids.end());
...
*/
			...
			log_msg( LOG_INFO, "Process %d started operating at socket %d",getpid(),uconn->sock );
        ...
        while ( ACTIVE ) // wait for new client
        {
            ...
            { // new client?
				...
				else
				{
					...
					if(pid < 0)
					{
						 log_msg( LOG_ERROR, "CHild was not created,error %d:%s",pid,strerror(pid) );
						
					}
                    else if(pid ==0)
                    {
                        close(sock_listen);
                        handleclient((void*)conn);
                        exit(0);
                    }
                    else if (pid >0)
                    {
