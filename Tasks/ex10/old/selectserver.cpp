...
typedef struct
{
	...
	//intialization
vector<connection_t*> inetcons;
vector<uconnection_t*> unetcons;
...
*/
            ...
            inetcons.erase(remove(inetcons.begin(), inetcons.end(), conn), inetcons.end());
            free(conn);
return NULL;
...
*/
            ...
            unetcons.erase(remove(unetcons.begin(), unetcons.end(), uconn), unetcons.end());
            free(uconn);
return NULL;
...
ErrorCheck(bindres);
    ...
    while ( ACTIVE )
    {
        ...
        fd_set inetset;
        for(unsigned int i = 0;i<inetcons.size();i++)
        {
            ...
            // add listen socket
            FD_SET( inetcons.at(i)->sock, &inetset );
            
            
            int sel = select( inetcons.at(i)->sock + 1, &inetset, NULL, NULL, NULL );
            ErrorCheck(sel);
            if ( sel < 0 )
            {
                log_msg( LOG_ERROR, "Select failed!" );
            ...
            if(FD_ISSET( inetcons.at(i)->sock, &inetset ))
            {
                handleclient((void*)inetcons.at(i));
                
            }
        }
        
                fd_set unetset;
        for(unsigned int i = 0;i<unetcons.size();i++)
        {
            ...
            // add listen socket
            FD_SET( unetcons.at(i)->sock, &unetset );
            
            
            int sel = select( unetcons.at(i)->sock + 1, &unetset, NULL, NULL, NULL );
            ErrorCheck(sel);
            if ( sel < 0 )
            {
                log_msg( LOG_ERROR, "Select failed!" );
            ...
            if(FD_ISSET( unetcons.at(i)->sock, &unetset ))
            {
                handleunixclient((void*)unetcons.at(i));
