...
int main( int argn, char **arg )
{
    ...
    while ( 1 )
    {
        ...
        { // communication
            char buf[ 256 ];
            // set for handles
            fd_set read_wait_set, exc_wait_set;
            // empty set
            FD_ZERO( &read_wait_set );
            FD_ZERO( &exc_wait_set );
            // add stdin
            FD_SET( STDIN_FILENO, &read_wait_set );
            // add client
            //FD_SET( sock_client, &read_wait_set );
            FD_SET( sock_client, &exc_wait_set );

            int sel = select( sock_client + 1, &read_wait_set, NULL, &exc_wait_set, NULL );
            ...
            // data from client?
            else if ( FD_ISSET( sock_client, &exc_wait_set ) )
            {
                // read data from socket
                int l = recv( sock_client, buf, sizeof( buf ), MSG_OOB );
                if ( !l )
                {
                        ...
                        log_msg( LOG_DEBUG, "Read OOB %d bytes from client.", l );
